...
if ( ! class_exists( 'WPCleverDashboard' ) ) {
    class WPCleverDashboard {
        ...
        function ajax_export() {
            ...
            $name = sanitize_text_field( $_POST['name'] ?? 'settings' );
            ...
            if ( ! empty( $key ) && ( $settings = get_option( $key ) ) ) {
                ...
                echo '<span style="color: #ff4f3b; font-size: 10px; margin-left: 10px">* Current ' . $name . ' will be replaced after pressing Update!</span>';
                echo '</div>';
        ...
        function ajax_import() {
            ...
            $key      = sanitize_key( $_POST['key'] ?? '' );
            $settings = sanitize_textarea_field( $_POST['settings'] ?? '' );
