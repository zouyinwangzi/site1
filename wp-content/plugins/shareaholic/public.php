...
class ShareaholicPublic {
	...
	public static function shortcode( $attributes, $content = null ) {
		...
		if ( isset( $attributes['id'] ) ) {
			$id = esc_attr( trim( $attributes['id'] ) );
		}
		if ( isset( $attributes['id_name'] ) ) {
			$id_name = esc_attr( trim( $attributes['id_name'] ) );
		}
		if ( isset( $attributes['app'] ) ) {
			$app = esc_attr( trim( $attributes['app'] ) );
		}
		if ( isset( $attributes['title'] ) ) {
			$title = esc_attr( trim( $attributes['title'] ) );
		}
		if ( isset( $attributes['link'] ) ) {
			$link = esc_url( trim( $attributes['link'] ) );
		}
	...
	public static function canvas( $app, $id = null, $id_name = null, $title = null, $link = null, $summary = null ) {
		...
		if ( $title == null && ( ! $is_list_page || $in_loop ) ) {
			$title = htmlspecialchars( $post->post_title, ENT_QUOTES );
		}
		if ( $link == null && ( ! $is_list_page || $in_loop ) ) {
			$link = get_permalink( $post->ID );
		}
		if ( $summary == null && ( ! $is_list_page || $in_loop ) ) {
	...
	public static function permalink_info() {
		...
		if ( isset($post->post_title) ) {
			$title                              = trim( html_entity_decode( $post->post_title ) );
			$post_content_title_character_count = strlen( $title );
			$post_content_title_word_count      = str_word_count( $title );
		} else {
			$post_content_title_character_count = 0;
			$post_content_title_word_count      = 0;
		}
		...
		if ( isset($post_body) ) {
			...
			$post_content_body_character_count = 0;
			$post_content_body_word_count      = 0;
		}
		...
		$info = array(
			...
			'post_stats'    => array(
				'post_comments_count'                => intval(get_comments_number( $post_id )),
				'post_content_title_character_count' => $post_content_title_character_count,
				'post_content_title_word_count'      => $post_content_title_word_count,
				'post_content_body_character_count'  => $post_content_body_character_count,
				'post_content_body_word_count'       => $post_content_body_word_count,
