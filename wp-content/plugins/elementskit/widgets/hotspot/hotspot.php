...
class ElementsKit_Widget_hotspot extends Widget_Base {
    ...
    protected function render_raw( ) {
		...
		foreach($active_breakpoints as $key => $breakpoint) {
			$breakpoints[$key] = $breakpoint->get_value();
		}
		?>

        <div class="ekit-location-groups" data-breakpoints="<?php echo esc_attr( json_encode($breakpoints) ); ?>">
            <div class="ekit-map-image text-center">
                <?php echo \Elementskit_Lite\Utils::get_attachment_image_html($settings, 'ekit_hotspot_background_image', 'full' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
            </div>
