...
class ElementsKit_Widget_Behance_Feed extends Widget_Base {
	...
	public function __construct($data = [], $args = null) {
		...
		if(Plugin::$instance->editor->is_edit_mode()) {
			wp_enqueue_script(
				'ekit-behance-feed-script-handle',
				Handler::get_url() . 'assets/js/script.js',
				['elementor-editor'],
    ...
    private function control_icon( $name, $selector, $config ) {
		...
		$this->add_control(  $key . 'heading', [
			'label'     => esc_html__(ucwords($name), 'elementskit'), //phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
			'type'      => Controls_Manager::HEADING
        ]);
    ...
    private function control_section_hover_overlay(){
		...
		$this->add_control( 'arrow_icon_size', [
            ...
            'selectors' => [
                '{{WRAPPER}} .ekit-feed-item-behance .ekit-feed-item--cover .ekit-feed-item--go-arrow a :is(i, svg)' => 'font-size: {{SIZE}}{{UNIT}};',
            ],
        ]);
        ...
        $this->add_control( 'arrow_icon_color', [
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .ekit-feed-item-behance .ekit-feed-item--cover .ekit-feed-item--go-arrow a :is(i, svg)' => 'color: {{VALUE}}; fill: {{VALUE}}',
				],
			]
		);
