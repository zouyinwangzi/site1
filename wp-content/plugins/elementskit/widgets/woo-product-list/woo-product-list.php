...
class ElementsKit_Widget_Woo_Product_List extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			'ekit_pagination_type',
			[
				'label'   => esc_html__( 'Pagination Type', 'elementskit' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'numbers',
				'options' => [
					'numeric' => esc_html__('Numeric', 'elementskit' ),
		...
		$this->add_responsive_control(
			...
			[
				...
				'type'      => Controls_Manager::SLIDER,
				'selectors' => [
					'{{WRAPPER}} ul.ekit-pagination li a i' => 'height: {{SIZE}}px; width: {{SIZE}}px;',
				],
			]
		);
	...
	public function render_loop_item() {
		...
		if($wp_query->have_posts() && post_type_exists('product')) {
			...
			<?php
				if ( 'yes' == $settings['ekit_show_pagination'] ) {
					echo $this->render_pagination($wp_query);
				}
			wp_reset_postdata();
		} else {
	 ...
	 * @example https://gist.github.com/raazon/04605b73c747266221c29a0f061f1bc5
	 * @param $the_query
	 * @return string
	 * @since 3.7.0
	 */
	public function render_pagination($the_query = null){
		$settings = $this->get_settings();
		$page_number = !empty($the_query->query['paged']) ? $the_query->query['paged'] :  $the_query->query_vars['paged'];
		$pagination_type = $settings['ekit_pagination_type'];
		$control_args = [];

		// Determine pagination style
		switch ($pagination_type) {
			case 'numeric':
				$control_args['prev_text'] = '<i class="eicon-chevron-left"></i>';
				$control_args['next_text'] = '<i class="eicon-chevron-right"></i>';
				break;
			case 'default':
				$control_args['prev_text'] = 'Prev';
				$control_args['next_text'] = 'Next';
				break;
			default:
				$control_args['prev_text'] = '<i class="eicon-chevron-left"></i>';
				$control_args['next_text'] = '<i class="eicon-chevron-right"></i>';
		...
		if ($the_query->max_num_pages > 1) {
			...
			$items = paginate_links(
				array(
					...
					'type'      => 'array',
					'prev_text' => $control_args['prev_text'],
					'next_text' => $control_args['next_text'],
					'end_size'  => 1,
					'mid_size'  => 1
				)
			);

			$pagination = "<ul class='ekit-pagination'>";
			$pagination .= implode('', array_map(fn($item) => "<li><span>{$item}</span></li>", (array)$items));
			$pagination .= "</ul>";
		}
		return $pagination;
	}
