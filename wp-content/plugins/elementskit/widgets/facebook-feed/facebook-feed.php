...
class ElementsKit_Widget_Facebook_Feed extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_responsive_control( 'overlay_background_hover', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .ekit-wid-con .ekit-single-fb-feed.image-in-background:hover::before' => 
					'background-image:linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, {{VALUE}} 100%);',
				'{{WRAPPER}} .ekit-wid-con .ekit-single-fb-feed.image-in-background:hover::after' => 
					'background-image:linear-gradient(180deg, {{VALUE}} 0%, {{VALUE}} 100%);',
			],
		]);
		...
		$this->add_responsive_control( 'image_padding', [
			...
			'selectors'  => [
				...
				'{{WRAPPER}} .ekit-single-fb-feed.image-in-background' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
		...
		$this->start_controls_section(
			...
			[
				...
				'tab'   => Controls_Manager::TAB_STYLE,
				'condition' => [
					'image_position' => ['left', 'background'],
				]
			]
		);
		...
		$this->start_controls_tabs('ekit_facebook_feed_footer_color_tabs');

		$this->start_controls_tab(
			'ekit_facebook_feed_footer_color_tab',
			[
				'label' => esc_html__('Normal', 'elementskit'),
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'default'   => '#606770',
				'selectors' => [
					'{{WRAPPER}} .ekit-fb-fotter-section .count'     => 'color: {{VALUE}}',
				],
			]
		);

		$this->add_responsive_control(
			'ekit_facebook_feed_footer_icon_color',
			[
				'label'     => esc_html__('Icon', 'elementskit'),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#606770',
				'selectors' => [
					'{{WRAPPER}} .ekit-fb-fotter-section i'     => 'color: {{VALUE}}',
					'{{WRAPPER}} .ekit-fb-fotter-section svg path'     => 'fill: {{VALUE}}',
				],
			]
		);
		...
		$this->start_controls_tab(
			'ekit_facebook_feed_footer_color_tab_hover',
			[
				'label' => esc_html__('Hover', 'elementskit'),
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'default'   => '#365899',
				'selectors' => [
					'{{WRAPPER}} .ekit-fb-fotter-section .count:hover'     => 'color: {{VALUE}}',
				],
			]
		);
		
		$this->add_responsive_control(
			'ekit_facebook_feed_footer_icon_color_hover',
			[
				'label'     => esc_html__('Icon', 'elementskit'),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#365899',
				'selectors' => [
					'{{WRAPPER}} .ekit-fb-fotter-section i:hover'     => 'color: {{VALUE}}',
					'{{WRAPPER}} .ekit-fb-fotter-section svg path:hover' => 'fill: {{VALUE}}',				
				],
			]
		);
	...
	protected function render_raw() {
		...
		if(!empty($page['param']['pg_id'])):
			...
			if(!empty($load['success'])) {
                ...
                <div class="ekit-facebook-feed">
                    <div class="ekit-fb-row <?php echo esc_attr($masnory_column . ' ' . $styleLayout); ?>">
						...
						foreach($response->data as $fbjson):
							...
							$cmt_link        = $fbjson->permalink_url?$fbjson->permalink_url : '#';
							...
							if($ekit_facebook_feed_style_choose == 'classic') : ?>
                                ...
                                <div class="ekit-single-fb-feed-holder <?php echo esc_attr($columnFeed); ?>">
									<div 
										...
										?>"
										style="<?php echo ($per_post_image_position == 'background' && !empty($feed_image)) ? 'background-image:url(' . esc_url($feed_image) . ')' : ''; ?>"
									>
                                        ...
                                        <p class="ekit-fb-feed-status">
											<?php echo wp_kses($this->str_check($fb_message), \ElementsKit_Lite\Utils::get_kses_array()) ;?>
										</p>
                                        ...
                                        <!--End Body -->
											...
											<?php if($show_reaction): ?>
												<div class="ekit-fb-fotter-section style-02">
													...
													<span class='ekit-fb-feed-share__share'>
														...
														<div class='ekit-fb-feed-share__menu'>
															...
															$twitter_params = 
																...
																'';
															?>
															<a href="<?php echo 'https://twitter.com/share' . esc_html($twitter_params); ?>" title="Share" target="_blank" class='ekit-fb-feed-share__with-tw'>
								...
								if(!empty($fb_full_picture)) : ?>
                                    <div class="ekit-single-fb-feed-holder <?php echo esc_attr($columnFeed); ?>">
                                        <div class="ekit-single-fb-feed <?php echo \ElementsKit\Utils::render($photos_class); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>">
				...
				echo esc_html__('Error msg : ', 'elementskit') . esc_html($load['msg']) . esc_html('<br/>');
			}
