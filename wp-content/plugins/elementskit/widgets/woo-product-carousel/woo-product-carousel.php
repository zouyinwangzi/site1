...
class ElementsKit_Widget_Woo_Product_Carousel extends Widget_Base {
		...
		<div class="ekit-wc-product-image ekit-wc-carousel-image ekit-background-cover">
			...
			?>
				<div class="ekit-wc-product-popop <?php echo esc_attr( $popupHorizontal_align ); ?> <?php echo esc_attr( $popupVertical_align ); ?>">
					<a class="ekit-wc-product-popop--link" href="<?php the_post_thumbnail_url( get_the_ID(), 'full' ); ?>"><i class="fas fa-eye" aria-hidden="true"></i></a>
				</div>
			<?php endif; ?>
			...
			?>
				<div class="ekit-position-cover ekit-overlay-default ekit-transition-<?php echo esc_attr($settings['ekit_overlay_animation']); ?>">
					...
					<?php
						...
						echo str_replace($product->add_to_cart_text(), '<span class="ekit-woo-add-cart-text">'. $product->add_to_cart_text() .'</span>', $add_cart_btn); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					?>
					</div>
				</div>
			<?php endif; ?>
			...
			<a href="<?php the_permalink(); ?>">
				<img src="<?php echo esc_url(wp_get_attachment_image_url(get_post_thumbnail_id(), $settings['ekit_image_size'])); ?>" alt="Product Thumb">
			</a>
		</div>
		<?php
	}
					...
					if($settings['ekit_show_categories'] === 'yes'){
						...
						if($terms_count > 0){
							...
							foreach($terms as $key => $term){
								...
								echo "<li><a href='". get_term_link($term->term_id) ."'>". esc_html( $term->name ) . esc_html( $sperator ) . "</a></li>"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							}
							echo "</ul></div>";
						}
					}
							...
							<div class="ekit-wc-rating ekit-flex-right ekit-width-expand">
								<?php
									if($product->get_rating_count() > 0){
										...
										echo \ElementsKit_Lite\Utils::render($rating_html); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									}
								?>
							</div>
						<?php endif; ?>
	...
	public function render_header() {
		...
		<div <?php $this->print_render_attribute_string( 'carousel' ); ?>>
			<div <?php $this->print_render_attribute_string( 'wc-carousel' ); ?>
			...
			>
				...
				<ul <?php $this->print_render_attribute_string( 'wc-carousel-wrapper' ); ?>>
		<?php
	}
	...
	public function render_loop_item() {
		...
		if($wp_query->have_posts()) {
			...
			while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
		  		<li <?php $this->print_render_attribute_string( 'wc-carousel-item' ); ?>>
