...
class ElementsKit_Widget_Video_Gallery extends Widget_Base {
	...
	protected function register_controls() {
		...
		// Self hosted
		$repeater->add_control(
			'is_self_video_custom_url',
			[
				'label' => esc_html__('Self Hosted Video URL', 'elementskit'),
		...
		$repeater->add_control(
			'self_hosted_video',
			[
				'label' => esc_html__( 'Choose Video', 'elementskit' ),
    ...
    protected function render_raw( ) {
        ...
        $config = [
            ...
            'slidesPerView'		=> !empty($slidetosho) ? $slidetosho['size'] : 4,
            ...
            'breakpoints'		=> [
                360 => [
                    'slidesPerView'     => !empty($slidetosho_mobile) ? $slidetosho_mobile['size'] : 1,
                    'slidesPerGroup'    => !empty($slidesToScroll_mobile) ? $slidesToScroll_mobile['size'] : 1,
                    'spaceBetween'      => !empty($left_right_spacing_mobile['size']) ? $left_right_spacing_mobile['size'] : 10,
                ],
                768 => [
                    'slidesPerView'     => !empty($slidetosho_tablet) ? $slidetosho_tablet['size'] : 2,
                    'slidesPerGroup'    => !empty($slidesToScroll_tablet) ? $slidesToScroll_tablet['size'] : 1,
                    'spaceBetween'      => !empty($left_right_spacing_tablet['size']) ? $left_right_spacing_tablet['size'] : 10,
                ],
                1024 => [
                    'slidesPerView'     => !empty($slidetosho) ? $slidetosho['size'] : 4,
                    'slidesPerGroup'    => !empty($slidesToScroll) ? $slidesToScroll['size'] : 1,
		...
		<div class="ekit-video-gallery-wrapper">
				...
				<ul class="elementskit-main-filter elementskit_filter_<?php echo esc_attr($ekit_gallery_filter_style_choose); ?>">
					...
					foreach($video_content as $nav) {
						if(!empty($nav['video_category']) && !in_array($nav['video_category'], $appended_cat)) : ?>
							<li>
								<a href='' data-value="<?php echo esc_attr($this->slugify($nav['video_category'])); ?>">
									<span class="elementskit_filter_nav_text"><?php echo esc_html($nav['video_category']); ?></span>
								</a>
							</li>
							<!-- // removed duplication -->
			...
			<div <?php echo \ElementsKit_Lite\Utils::render($this->get_render_attribute_string( 'wrapper' )); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
				...
				<?php foreach($video_content as $video) :
					...
					if($video_type == 'youtube') {
						if(strpos($video_url['url'], 'youtu.be') !== false) {
							$path = parse_url($video_url['url'], PHP_URL_PATH);
							$video_id = ltrim($path, '/');
							$watch_url = 'https://www.youtube.com/watch?v='.$video_id;
						} else{
							$query_string = parse_url( $video_url['url'], PHP_URL_QUERY );
							if (!empty($query_string)) {
								parse_str($query_string, $y_v_id);
								$video_id = ! empty( $y_v_id['v'] ) ? $y_v_id['v'] : '';
							} else {
								$video_id = ''; // fallback if no query
							}
						}
						...
						$video_thumb = empty($video_thumb) ? $this->getVimeoVideoThumb($video_id) : $video_thumb; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						$data_url = 'https://player.vimeo.com/video/'.$video_id.'?autoplay=1&version=3&enablejsapi=1';
					} else {
						$is_url = $video['is_self_video_custom_url'];
						$video_url = ($is_url == 'yes') ? $video['self_hosted_video_url'] : $video['self_hosted_video'];
						$data_url = ($is_url == 'yes') ? $video['self_hosted_video_url']['url'] : $video['self_hosted_video']['url'];
					}
				?>
					<div class="ekit-video-item <?php echo $grid_style == 'carousel' ? 'swiper-slide ' : ' '; echo $this->slugify(esc_attr($video['video_category'])); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>">

						<?php if($grid_style == 'carousel') : ?>
							<div class="swiper-slide-inner">
						<?php endif; ?>
								<?php if(strpos($video_url['url'], 'youtu.be') !== false) : ?>
									<a class="video-link <?php echo esc_attr($click_action); ?>" href="<?php echo esc_url($watch_url); ?>" data-url="<?php echo esc_url($data_url); ?>">
								<?php else : ?>
								...
								<?php endif; ?>
									...
									<?php Icons_Manager::render_icon( $settings['button_icons'], [ 'aria-hidden' => 'true', 'class'=>  'video-icon' ] ); ?>
								</a>
						<?php if($grid_style == 'carousel') : ?>
							</div>
						<?php endif; ?>
