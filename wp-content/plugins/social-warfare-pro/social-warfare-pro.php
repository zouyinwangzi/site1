...
add_filter(
	...
	function ( $pre, $parsed_args, $url ) {
		if ( strpos( $url, 'warfareplugins.com' ) === false ) {
			return $pre;
		}

		if ( ! isset( $parsed_args['body'] ) || ! isset( $parsed_args['body']['edd_action'] ) ) {
			return $pre;
		}

		$action = $parsed_args['body']['edd_action'];

		// Handle both version checks and license activation.
		if ( $action === 'get_version' || $action === 'activate_license' || $action === 'check_license' || $action === 'deactivate_license' ) {
			$license_key = isset( $parsed_args['body']['license'] ) ? $parsed_args['body']['license'] : 'dummy-key';
			$response    = array(
				'success'          => true,
				'license'          => 'valid',
				...
				'customer_email'   => 'ok@weadown.com',
				'license_limit'    => 0,
				'site_count'       => 1,
				'activations_left' => true,
				'price_id'         => 1,
				'item_id'          => SWPP_SL_PRODUCT_ID,
				'item_name'        => 'Social Warfare Pro',
				'author'           => 'Warfare Plugins',
				'sections'         => array(
					'description' => 'Social Warfare Pro',
					'changelog'   => 'Mock changelog',
				),
				'download_link'    => '',
				'new_version'      => SWPP_VERSION,
 ...
 *
 */
define( 'SWPP_VERSION', '4.5.6' );
define( 'SWPP_DEV_VERSION', '2024.07.29 MASTER' );
