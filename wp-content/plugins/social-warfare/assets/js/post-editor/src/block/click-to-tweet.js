...
const icon = (
	<div className="swp-block-icon" style={ { color: '#429cd6' } }>
...
registerBlockType( 'social-warfare/click-to-tweet', {
			...
			<div className="social-warfare-admin-block">
				...
				>
					...
					<p>
						...
						shortcode.{ ' ' }
						<a href="https://warfareplugins.com/support/click-to-tweet/">
							Learn more
						</a>
					</p>
					...
					<div style={ { width: '100%' } }>
						<p
							className={ `block-characters-remaining ${ className }` }
							style={ { marginTop: -33 } }
						>
	 ...
	 */
	save( props ) {
		const tweetText = props.attributes.tweetText;
		let displayText = props.attributes.displayText;
		if ( ! tweetText ) {
			return;
		}
		if ( ! displayText ) {
		...
		const theme = props.attributes.theme
			? `style${ props.attributes.theme }`
			: '';
		return (
			<div className="social-warfare-admin-block">
