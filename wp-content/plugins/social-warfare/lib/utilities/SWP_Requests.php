...
class SWP_Requests {
	...
	public static function fetch_shares_via_wordpress_multi( $links ) {
		...
		if ( class_exists( 'WpOrg\Requests\Requests' ) ) {
            ...
            $responses = array();
            foreach ( $request_multi as $key => $request ) {
                $resp = wp_remote_get( $request['url'] );
                if ( is_wp_error( $resp ) ) {
                    $responses[ $key ] = (object) array( 'body' => '' );
                } else {
                    $responses[ $key ] = (object) array( 'body' => wp_remote_retrieve_body( $resp ) );
                }
            }
        }
        ...
        $response = array();
        foreach ( $responses as $key => $object ) {
            if ( is_array( $object ) && isset( $object['body'] ) ) {
                $response[ $key ] = $object['body'];
            } elseif ( is_object( $object ) && isset( $object->body ) ) {
                $response[ $key ] = $object->body;
            } else {
                $response[ $key ] = '';
            }
